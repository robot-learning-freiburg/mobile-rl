...
from pybindings_robots import RobotPR2, RobotTiago, RobotHSR, RobotFMM
...
class RobotEnv(Env):
    def __init__(self,
                 ...
                 arm_selection: str = "franka_arm"):
        if env == 'fmm':
            conf_path = PROJECT_ROOT / "gazebo_world" / env / f"robot_config_{arm_selection}.yaml"
        else:
        ...
        if env == 'pr2':
            ...
            self.robot_config['ik_joint_model_group_name'] = arm_selection if learn_torso else self.robot_config['joint_model_group_name']
            self.drive = "omni"
            robot_fn = RobotFMM
        else:
    ...
    def get_joint_values(self, joint_model_group: Optional[str] = None):
        if joint_model_group is None:
            joint_model_group = self.robot_config['joint_model_group_name']
        return np.array(self._robot_cpp.get_joint_values(joint_model_group))
    
    def set_joint_values(self, joint_values, joint_model_group: Optional[bool] = False):
        return self._robot_cpp.set_joint_values(joint_values, joint_model_group)
    
    def reload_robot_description(self):
        self._robot_cpp.reload_robot_description()

    def get_manipulability(self, joint_model_group: Optional[str] = None):
        if joint_model_group is None:
            joint_model_group = self.robot_config['joint_model_group_name']
        return self._robot_cpp.get_manipulability(joint_model_group)
    
    def get_jacobian(self, joint_model_group: Optional[str] = None):
        if joint_model_group is None:
            joint_model_group = self.robot_config['joint_model_group_name']
        jacobian = self._robot_cpp.get_jacobian(joint_model_group)
        return jacobian
    ...
    def get_find_ik_python(self, target, dt_exec, in_selfcollision, threshold): 
        return self._robot_cpp.find_ik_python(target, dt_exec, in_selfcollision, threshold)
