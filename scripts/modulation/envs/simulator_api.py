...
def sample_circle_goal(base_tf: list, goal_dist_rng: Tuple[float, float], goal_height_rng: Tuple[float, float], map_width: float, map_height: float, np_random, distribution: str = "uniform") -> list:
    ...
    if z < HeightDefinitions.rndstartrndgoal_switch_borders[0]:
        # if goal is too low just grab it from top +- 45 degrees 
        RPY_goal[1] = np.random.uniform((1/4) * np.pi, (3/4) *np.pi)

    if z >  HeightDefinitions.rndstartrndgoal_switch_borders[1]:
        # if goal is too high just grab it from bottom and slightly from the front top to grasp something there
        # + 45 degrees - (-180) degree
        RPY_goal[1] = np.random.uniform((1/4) * np.pi, - np.pi)
