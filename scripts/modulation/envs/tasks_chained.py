...
from modulation.envs.simulator_api import WorldObjects, SpawnObject, DEFAULT_FRAME, DynObstacle, GazeboObject, ObjectGeometry
from modulation.envs.tasks import BaseTask, TaskGoal, GripperActions, armarker_pose_to_listtf, publish_goalpointer_enabled
from modulation.envs.height_definitions import HeightDefinitions
...
class PickNPlaceChainedTask(BaseChainedTask):
    ...
    def __init__(self, env: CombinedEnv, obstacle_configuration: str, use_fwd_orientation: bool, eval: bool, map=None):
        ...
        self.pnp_height_rng = HeightDefinitions.picknplace_range 
    ...
    def get_goal_objects(self, start_table_pos=Point(x=3.3, y=0, z=0), end_table_rng=(-1.5, 2, -3, -1.5)):
        objects = []
        pick_place_difference = 0 
        if self.pnp_height_rng is not None and len(self.pnp_height_rng) == 2:
            table_heights = self.np_random.uniform(self.pnp_height_rng[0], self.pnp_height_rng[1], 2)
            self._pick_table = GazeboObject('pick_table', 0.75, 0.75, table_heights[0], ObjectGeometry.RECTANGLE)
            self._place_table = GazeboObject('place_table', 0.75, 0.75, table_heights[1], ObjectGeometry.RECTANGLE)
            pick_place_difference = table_heights[1] - table_heights[0]
        ...
        end_table_pose = Pose(Point(x=endx, y=endy, z=pick_place_difference), Quaternion(0, 0, 1, 1))
    ...
    def draw_goal(self) -> List[TaskGoal]:
        ...
        obj_loc = [world_target_pos.x, world_target_pos.y, world_target_pos.z - 0.04] + self.get_orientation(pick_task=True, height=world_target_pos.z)
        in_front_of_obj_loc = translate_in_orientation(obj_loc, [-0.2, 0., 0.])

        # place goals
        world_end_target_pos = self.place_obj_pose.position
        place_loc = [world_end_target_pos.x, world_end_target_pos.y + 0.05, world_end_target_pos.z + 0.05] + self.get_orientation(pick_task=False, height=world_end_target_pos.z)
    ...
    def front_grasp_orientation(self):
        return 0

    def top_grasp_orientation(self):
        return np.pi / 2
    
    def pick_orienation(self):
        return 0

    def place_orientation(self):
        return - (np.pi / 2)

    def get_orientation(self, pick_task: bool, height: float):
        task_orienation = self.pick_orienation() if pick_task else self.place_orientation()
        if height < HeightDefinitions.picknplace_switch_borders[0]:
            pitch = self.top_grasp_orientation() 
        elif height > HeightDefinitions.picknplace_switch_borders[1]:
            pitch = self.front_grasp_orientation()
        else:
            pitch = self.np_random.choice([self.front_grasp_orientation(), self.top_grasp_orientation()])
        
        return [0, pitch, task_orienation]
...
class DoorChainedTask(BaseChainedTask):
    ...
    def get_goal_objects(self, shelf_pos=Point(x=0.0, y=3.3, z=0.24)):
        ...
        # randomize height of door nod
        border = HeightDefinitions.cabinet_range
        p.z = self.np_random.uniform(border[0], border[1])
...
class DrawerChainedTask(BaseChainedTask):
    ...
    def get_goal_objects(self, drawer_pos=Point(x=-3.3, y=0.0, z=0.24)) -> List[SpawnObject]:
        ...
        # randomize height of drawer
        border = HeightDefinitions.drawer_range
        p.z = self.np_random.uniform(border[0], border[1])
