...
def get_design_parser():
    '''    
    Returns the parser for the design optimization script

    '''
    parser = argparse.ArgumentParser()
    
    parser.add_argument('--training_steps', type=int, default=100, help='Number of training steps for policy optimization after to stop after')
    parser.add_argument('--resume', type=str2bool, nargs='?', const=True, default=False, help='Resume from previous run')
    parser.add_argument('--log_path', type=str, default='scripts/logs/design', help='Path to save the design corresponding logs')
    parser.add_argument('--cpu', type=str2bool, nargs='?', const=True, default=False, help='Use CPU instead of GPU for design optimization')
    parser.add_argument('--debug', type=str2bool, nargs='?', const=True, default=False, help='Debug mode')
    # parser.add_argument('--population_size', type=int, default=10)
    # parser.add_argument('--rand_start_cnt', type=int, default=5)
    # parser.add_argument('--mutation_cnt', type=int, default=15)
    # parser.add_argument('--metric', type=str, default='episode_reward_mean')
    # BOHB Framework
    parser.add_argument('--min_budget', type=int, default=1000, help='Minimum budget for the BOHB framework')
    parser.add_argument('--max_budget', type=int, default=15000, help='Maximum budget for the BOHB framework')
    parser.add_argument('--n_iterations', type=int, default=1, help='Number of iterations for the BOHB framework')
    parser.add_argument('--num_samples', type=int, default=1, help='Number of samples for the BOHB framework')
    parser.add_argument('--min_points_in_model', type=int, default=None, help='Minimum number of points in the model')
    parser.add_argument('--bandwidth_factor', type=int, default=3, help='Bandwidth factor for the BOHB framework')
    parser.add_argument('--eta', type=int, default=3, help='Eta for the BOHB framework')
    parser.add_argument('--n_workers', type=int, default=1, help='Number of workers for the BOHB framework')
    parser.add_argument('--previous_run_dir', type=str, default=None, help='Directory of the previous run to continue from -> so called warmstart')
    # other configuration
    parser.add_argument('--arm_selection', type=str, default='franka_arm', choices=['franka_arm', 'ur5_arm'], help="Arm selection of the robot")
    parser.add_argument('--optimization_type', type=str, default='design', choices=['design', 'manipulability'], help='Defines the optimization metric, using the manipulability or the evaluated design')
    parser.add_argument('--manipulability_design_configs_path', type=str, default='scripts/design_finding/manipulability/manipulability_configs.yaml', help='Path to the best manipulability design configurations')
    parser.add_argument('--use_manipulability_config', type=str, default=None, help='Use the manipulability configs to train an RL agent on')
...
def get_parser():
    ...
    # Env
    #################################################
    parser.add_argument('--env', type=str.lower, default='pr2', choices=['pr2', 'tiago', 'hsr', 'fmm'], help='')
...
def parse_args(config_path, framework: str, using_wandb: bool = False, raw_args=None, use_config = False, add_design_config=False):
    ...
    if not use_config:
        ...
        args = vars(args)
        if add_design_config:
            design_parser = get_design_parser()
            design_config = design_parser.parse_args(raw_args)
            design_config = vars(design_config)
    ...
    if add_design_config and design_config is not None:
        new_args = {}
        new_args['ray_config'] = args
        new_args['design_config'] = design_config
        return run_name, group, new_args, cl_args
...
def launch_ros(main_path: Path, config, task: str, no_gui: bool = False, always_relaunch: bool = None, algo="rl", pure_analytical=None):
    ...
    out = subprocess.Popen(["python2", script] + args, env=my_env)
    out.wait()
...
def setup_config_wandb(config_path, framework: str, sync_tensorboard=False, allow_init: bool = True,
    ...
    if args['resume_id']:
        ...
        if args['model_file'].startswith('scripts/logs/design'):
            # already the absolute path to the checkpoint file
            model_file = args['model_file']
        else:
            model_file = config_path / 'model_checkpoints' / args["env"] / args['model_file']
...
def create_env(config,
    ...
    robot_env = RobotEnv(env=config["env"],
                         ...
                         arm_selection=config.get("arm_selection", "franka_arm"))
