...
PYBIND11_MODULE(pybindings_robots, m) {
    ...
    robotenv.def("reset", &RobotEnv::reset, "Reset.")
        ...
        .def("get_manipulability", &RobotEnv::getManipulability, "getManipulability.")    
        .def("get_jacobian", &RobotEnv::getJacobian, "getJacobian.")    
        .def("reload_robot_description", &RobotEnv::reloadRobotDescription, "reloadRobotDescription.")    
        .def("find_ik_python", &RobotEnv::findIkPython, "findIkPython.");
    ...
    py::class_<RobotFMM>(m, "RobotFMM", robotenv /* <- specify Python parent type */)
        .def(py::init<uint32_t, std::string, bool, std::string, RoboConf, double, double, double, std::string>());
