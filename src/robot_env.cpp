...
RobotEnv::RobotEnv(uint32_t seed,
    ...
    // TODO: hacky, fmm doesn't have base_footprint :(
    if (robot_config_.name == "fmm"){
    ...
    floor_object.header.frame_id = base_link_name_;
...
// Function to reload robot description
void RobotEnv::reloadRobotDescription() {
    // Reload robot description
    robot_model_loader::RobotModelLoader robot_model_loader("robot_description");
    robot_model::RobotModelPtr robot_model = robot_model_loader.getModel();
    kinematic_state_.reset(new robot_state::RobotState(robot_model));
    // set kinematic chain to a random initial position
    // kinematic_state_->setToRandomPositions(joint_model_group_, *rng_);
    // Use the new model to initialize a RobotState or any other dependent objects
    // moveit::core::RobotStatePtr kinematic_state(new moveit::core::RobotState(kinematic_model));
}


double RobotEnv::getManipulability(const std::string &joint_group){
    /*
        From MANIPULABILITY AND REDUNDANCY CONTROL OF ROBOTIC MECHANISMS
        TSUNEO YOSHIKAWA
        https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=1087283
    */
    ...
    Eigen::MatrixXd jac_multi = jacobian * jacobian_transpose;
    // Calculate determinant of the result matrix
    double determinant = jac_multi.determinant();
    if (determinant < 0){
        cout << "Determinant is negative, so returning 0" << endl;
        return 0;
    }
    double manipulability = sqrt(determinant);
    // double manipulability = 2.0;
    // Take the square root of the determinant
    return manipulability;
}

std::vector<std::vector<double>> eigenToVector(const Eigen::MatrixXd &matrix) {
...
Eigen::MatrixXd RobotEnv::getJacobian(const std::string &joint_group){
    const robot_state::JointModelGroup *joint_model_group = kinematic_state_->getJointModelGroup(joint_group);
    Eigen::Vector3d reference_point_position(0.0, 0.0, 0.0);
    Eigen::MatrixXd jacobian;
    kinematic_state_->getJacobian(joint_model_group,
...
bool RobotEnv::findIkPython(const std::vector<double> &desired_gripper_tf_world, const double dt, bool &in_selfcollision, double threshold){
    // set kinematic chain to a random initial position
    kinematic_state_->setToRandomPositions(joint_model_group_, *rng_);
    tf::Transform desired_gripper_tf = myutils::listToTf(desired_gripper_tf_world);
    return findIk(desired_gripper_tf, dt, in_selfcollision, threshold);
}
...
bool RobotEnv::findIk(const tf::Transform &desired_gripper_tf_world, const double dt, bool &in_selfcollision, double threshold) {
    vector<double> joint_values_pre;
    kinematic_state_->copyJointGroupPositions(ik_joint_model_group_, joint_values_pre);
    ...
    tf::Vector3 gripper_distance = gripper_tf.getOrigin() - desired_gripper_tf_world.getOrigin();
    float distance = gripper_distance.length();
    // std::cout << "Distance: " << distance << std::endl;
    // py::print(distance);
    if (distance > threshold){
        // ("IK solution is too far away from the current pose");
        success = false;
    }
    return success;
}
