...
GaussianMixtureModel::GaussianMixtureModel(double max_speed_base_rot) :
...
GaussianMixtureModel::~GaussianMixtureModel() = default;

tf::Transform GaussianMixtureModel::tipToObjOrigin(const tf::Transform &tip) const {
    tf::Transform related_object_pose = getRelatedObjPose();
    tf::Transform last_mu_gripper_orig = getLastMuEigenBckGripper();
    return tip * last_mu_gripper_orig.inverse() * related_object_pose;
}

std::vector<double> GaussianMixtureModel::objOriginToTip(const std::vector<double> &obj_origin) const {
    tf::Transform obj_origin_tf(myutils::listToTf(obj_origin));
    tf::Transform related_object_pose = getRelatedObjPose();
    tf::Transform last_mu_gripper_orig = getLastMuEigenBckGripper();
    return myutils::tfToList(obj_origin_tf * related_object_pose.inverse() * last_mu_gripper_orig);
}

std::vector<std::vector<double>> GaussianMixtureModel::getMusPython() const{
    int nrModes = getNrModes();
    std::vector<std::vector<double>> v;

    for (int i = 0; i < nrModes; i++) {
        std::vector<Eigen::VectorXf> muEigen = getMu();
        ...
        v.push_back(myutils::tfToList(gripper_t));
        v.push_back(myutils::tfToList(base_t));
    }
    return v;
}

void GaussianMixtureModel::AdaptModelPython(const std::vector<double> &gripper_goal_tip, const std::vector<double> &gmm_base_offset){
    tf::Transform obj_origin_goal = tipToObjOrigin(myutils::listToTf(gripper_goal_tip));
    adaptModel(obj_origin_goal, myutils::listToVector3(gmm_base_offset));
}
...
void GaussianMixtureModel::adaptModel(tf::Transform obj_origin_goal, tf::Vector3 gmm_base_offset) {
    ...
    for (int i = 0; i < _nr_modes; i++) {
        ...
        tf_Mu_base_i.setRotation(tf::Quaternion(0.0,
                                                0.0,
        ...
        // base goal was not necessarily learned for this robot. So use an offset to ensure robot moves close enough to the final goal
        tf_Mu_base_i = myutils::tipToGripperGoal(tf_Mu_base_i, -gmm_base_offset, tf::Quaternion(0, 0, 0, 1));

        // set _MuEigen
        double time_i = _MuEigenBck[i](0);
        Eigen::VectorXf Mu_i_eigen(15);
        Mu_i_eigen << time_i, tf_Mu_gr_i.getOrigin().x(), tf_Mu_gr_i.getOrigin().y(), tf_Mu_gr_i.getOrigin().z(), tf_Mu_gr_i.getRotation().x(), tf_Mu_gr_i.getRotation().y(),
            tf_Mu_gr_i.getRotation().z(), tf_Mu_gr_i.getRotation().w(), tf_Mu_base_i.getOrigin().x(), tf_Mu_base_i.getOrigin().y(), tf_Mu_base_i.getOrigin().z(),
...
std::vector<double> GaussianMixtureModel::integrateModelPython(double current_time,
                                                               double dt_real,
                                                               ...
                                                               const double gmm_time_offset,
                                                               const std::vector<double> &tip_to_gripper_offset) {
    tf::Transform current_gripper_tf_tip = myutils::gripperToTipGoal(myutils::listToTf(current_gripper_tf_wrist),
                                                                   myutils::listToVector3(tip_to_gripper_offset),
                                                                   tf::Quaternion(0., 0., 0., 1.));
    tf::Transform current_base_tf = myutils::listToTf(current_base_tf_list);

    Eigen::VectorXf current_pose(14);
    current_pose << current_gripper_tf_tip.getOrigin().x(), current_gripper_tf_tip.getOrigin().y(), current_gripper_tf_tip.getOrigin().z(),
                    current_gripper_tf_tip.getRotation().x(), current_gripper_tf_tip.getRotation().y(), current_gripper_tf_tip.getRotation().z(), current_gripper_tf_tip.getRotation().w(),
                    current_base_tf.getOrigin().x(), current_base_tf.getOrigin().y(), 0.0,
                    current_base_tf.getRotation().x(), current_base_tf.getRotation().y(), current_base_tf.getRotation().z(), current_base_tf.getRotation().w();
    Eigen::VectorXf current_speed_vector(14);
    current_speed_vector << current_speed[0], current_speed[1], current_speed[2], current_speed[3], current_speed[4],
                            current_speed[5], current_speed[6], current_speed[7], current_speed[8], current_speed[9],
                            current_speed[10], current_speed[11], current_speed[12], current_speed[13];

    integrateModel(current_time, dt_real, &current_pose, &current_speed_vector, min_velocity, max_velocity, max_rot_velocity, gmm_time_offset);

//    std::vector<double> vec(reinterpret_cast<unsigned long>(current_pose.data()), current_pose.size());

    tf::Transform next_gripper_tf_tip;
    next_gripper_tf_tip.setOrigin(tf::Vector3(current_pose[0], current_pose[1], current_pose[2]));
    next_gripper_tf_tip.setRotation(tf::Quaternion(current_pose[3], current_pose[4], current_pose[5], current_pose[6]));
    tf::Transform next_gripper_tf_wrist = myutils::tipToGripperGoal(next_gripper_tf_tip, myutils::listToVector3(tip_to_gripper_offset), tf::Quaternion(0., 0., 0., 1.));

    // next_gripper_tf: [0:7], next_base_tf: [7:14]
//    std::vector<double> vec;
//    for (int i=0;i<14; i++){
//        vec.push_back(current_pose[i]);
//    }

    return myutils::tfToList(next_gripper_tf_wrist);

//    tf::Transform next_gripper_tf;
//    next_gripper_tf.setOrigin(tf::Vector3(current_pose[0], current_pose[1], current_pose[2]));
//    next_gripper_tf.setRotation(tf::Quaternion(current_pose[3], current_pose[4], current_pose[5], current_pose[6]));
...
//    return myutils::tfToList(next_gripper_tf);
}

void GaussianMixtureModel::integrateModel(double current_time,
                                          double dt_real,
    ...
    current_time_gmm -= gmm_time_offset / _motion_duration;
    ...
    tf::Quaternion desired_gripper_q =
        tf::Quaternion(_MuEigen[highest_i](4), _MuEigen[highest_i](5), _MuEigen[highest_i](6), _MuEigen[highest_i](7))
            .slerp(tf::Quaternion(_MuEigen[secHighest_i](4), _MuEigen[secHighest_i](5), _MuEigen[secHighest_i](6), _MuEigen[secHighest_i](7)), h_ratio);
    ...
    tf::Quaternion current_desired_base_q =
        tf::Quaternion(0.0, 0.0, _MuEigen[highest_i](13), _MuEigen[highest_i](14)).slerp(tf::Quaternion(0.0, 0.0, _MuEigen[secHighest_i](13), _MuEigen[secHighest_i](14)), h_ratio);
    ...
    *current_speed = *current_speed + 1 * currAcc;
    ...
    gripper_vel = myutils::normScaleVel(gripper_vel, min_velocity * dt_real, max_velocity * dt_real);
    current_speed->coeffRef(0) = gripper_vel.x();
    current_speed->coeffRef(1) = gripper_vel.y();
    current_speed->coeffRef(2) = gripper_vel.z();
    //	if(std::abs(current_speed->coeffRef(0)) >max_velocity || std::abs(current_speed->coeffRef(1)) >max_velocity || std::abs(current_speed->coeffRef(2)) >max_velocity){
    ...
    gripper_rot_vel = myutils::normScaleVel(gripper_rot_vel, 0.0, max_rot_velocity * dt_real);
    ...
    //	if(std::abs(current_speed->coeffRef(3)) >_max_speed_gripper_rot || std::abs(current_speed->coeffRef(4)) >_max_speed_gripper_rot || std::abs(current_speed->coeffRef(5))
    //>_max_speed_gripper_rot){ 		double max_element = std::max(std::abs(current_speed->coeffRef(3)),std::abs(current_speed->coeffRef(4))); 		max_element =
    //std::max(max_element,std::abs((double)current_speed->coeffRef(5))); 		current_speed->coeffRef(3) = current_speed->coeffRef(3)/max_element*_max_speed_gripper_rot;
    //		current_speed->coeffRef(4) = current_speed->coeffRef(4)/max_element*_max_speed_gripper_rot;
    //		current_speed->coeffRef(5) = current_speed->coeffRef(5)/max_element*_max_speed_gripper_rot;
    //	}

    // limit robotbase speed
    // 'hack' to ensure that if starting far away the base won't out-run the gripper (due to z always being 0 in the norm-scaling):
    // scale with the same z as the gripper, then set z to zero
    tf::Vector3 base_vel(current_speed->coeffRef(7), current_speed->coeffRef(8), unscaled_gripper_z);
    base_vel = myutils::normScaleVel(base_vel, min_velocity * dt_real, max_velocity * dt_real);
    base_vel.setZ(0.0);
    ...
    current_speed->coeffRef(12) = myutils::clampDouble(current_speed->coeffRef(12), -lim, lim);
    //	if(std::abs(current_speed->coeffRef(12)) >_max_speed_base_rot){
    //		current_speed->coeffRef(12) = current_speed->coeffRef(12) / (std::abs(current_speed->coeffRef(12)) * _max_speed_base_rot);
    //	}
    ...
    // std::cout << "current_speed*dt_real->coeffRef(0:2) post: " << (*current_speed)(0) << ", " << (*current_speed)(1)<< ", " << (*current_speed)(2) << std::endl;
...
template<typename T> bool GaussianMixtureModel::parseVector(std::ifstream &is, std::vector<T> &pts, const std::string &name) {
    ...
    std::string nameStr;
    ss >> nameStr;
    nameStr = myutils::trim(nameStr);
    if (myutils::startsWith(nameStr, "\"")) {
        nameStr = nameStr.substr(1);
        if (myutils::endsWith(nameStr, "\"")) {
            ...
            while (ss.good()) {
                std::string discard;
                ss >> discard;
                discard = myutils::trim(discard);
                if (myutils::endsWith(discard, "\""))
                    break;
            }
        }
    }
    if (!myutils::startsWith(nameStr, name)) {
...
bool GaussianMixtureModel::loadFromFile(std::string &filename) {
    ...
    do {
        ...
        line = myutils::trim(line);
    } while (line.empty());

    if (!myutils::startsWith(line, "\"")) {
        ROS_ERROR("Name start error in %s", line.c_str());
        return false;
    }
    if (!myutils::endsWith(line, "\"")) {
    ...
    for (int i = 0; i < _nr_modes; i++) {
        ...
        _Mu.push_back(Mu_i);
        Eigen::VectorXf Mu_i_eigen(15);
        Mu_i_eigen << Mu_i[0], Mu_i[1], Mu_i[2], Mu_i[3], Mu_i[4], Mu_i[5], Mu_i[6], Mu_i[7], Mu_i[8], Mu_i[9], Mu_i[10], Mu_i[11], Mu_i[12], Mu_i[13], Mu_i[14];
        _MuEigen.push_back(Mu_i_eigen);
    }
    _MuEigenBck = _MuEigen;
    // Read in Sigma
