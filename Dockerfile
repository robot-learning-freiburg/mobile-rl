...
#NOTE: requires to have sensor-to-cloud as a submodule (git submodule update(?))
...
RUN git clone https://github.com/robot-learning-freiburg/fmm_description.git src/fmm_description \
    ...
    && git checkout additional_link
    # && git checkout UR5-arm 

# UR5 files
RUN git clone https://github.com/ros-industrial-attic/robot_movement_interface.git src/ur5
RUN git clone -b melodic-devel https://github.com/GT-RAIL/robotiq_85_gripper.git src/robotiq_gripper
...
COPY gazebo_world catkin_ws_fmm/src/modulation_rl/gazebo_world/
...
COPY GMM_models/ catkin_ws_fmm/src/modulation_rl/GMM_models/
...
COPY rviz/rviz_config.rviz catkin_ws_fmm/src/modulation_rl/rviz/rviz_config.rviz
...
COPY .git/ catkin_ws_fmm/src/modulation_rl/.git/
